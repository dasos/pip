...
object WearPaths {
    ...
    const val SYNC_REQUEST_PATH = "/pip/sync-request"
    ...
    const val KEY_SYNC_REQUEST = "sync_request"
}

suspend fun requestWatchSync(context: Context) {
    val putReq = PutDataMapRequest.create(WearPaths.SYNC_REQUEST_PATH).apply {
        dataMap.putLong(WearPaths.KEY_SYNC_REQUEST, System.currentTimeMillis())
    }
    runCatching {
